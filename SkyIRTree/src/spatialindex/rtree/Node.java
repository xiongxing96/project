...
abstract class Node implements INode
{
	...
	protected int m_level = -1;
		// The level of the node in the tree.//层级
		// Leaves are always at level 0.

	protected int m_identifier = -1;
		// The unique ID of this node.节点id

	protected int m_children = 0;
		// The number of children pointed by this node.//孩子个数

	protected int m_capacity = -1;
		// Specifies the node capacity.//节点容量

	protected Region m_nodeMBR = null;
		// The minimum bounding region enclosing all data contained in the node.//节点所包括的最小矩形

	protected byte[][] m_pData = null;
		// The data stored in the node.//

	protected Region[] m_pMBR = null;
		// The corresponding data MBRs.//全体数据矩形

	protected int[] m_pIdentifier = null;
		// The corresponding data identifiers.全体数据id
	...
	protected boolean insertData(byte[] pData, Region mbr, int id, Stack pathBuffer, boolean[] overflowTable)
	{
		...
		{
			Node[] nodes = split(pData, mbr, id);//分裂节点
			Node n = nodes[0];
			Node nn = nodes[1];
