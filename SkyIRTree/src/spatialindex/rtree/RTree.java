...
public class RTree implements ISpatialIndex
{
	...
	public RTree(PropertySet ps, IStorageManager sm)
	{
		...
		System.out.println("m_treeVariant="+m_treeVariant);
		m_fillFactor = 0.7f;
		m_indexCapacity = 100;
		m_leafCapacity = 100;
	...
	private void initNew(PropertySet ps) throws IOException
	{
		...
		System.out.println("TreeVariant"+var);
		if (var != null)
		{
		...
		System.out.println("FillFactor"+var);
		if (var != null)
		{
	...
	private void storeHeader() throws IOException
	{
		...
		System.out.println("m_stats.m_treeHeight="+m_stats.m_treeHeight+";;m_treeVariant"+m_treeVariant);
		for (int cLevel = 0; cLevel < m_stats.m_treeHeight; cLevel++)
		{
	...
	protected void insertData_impl(byte[] pData, Region mbr, int id, int level, boolean[] overflowTable)
	{
		...
		Node n = root.chooseSubtree(mbr, level, pathBuffer);// 寻找子树
		n.insertData(pData, mbr, id, pathBuffer, overflowTable);// 插入数据
	}
